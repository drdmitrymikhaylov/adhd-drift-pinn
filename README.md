...
**A physics-informed solver for the decision equation, applied to 79 children — and an honest account of what it does not buy you.**
...
> decision boundary. That first-passage time obeys a differential equation.
> This repository solves that equation with a neural network, fits the solution
> to every child in an open dataset, and then asks the question most papers of
> this kind skip: **does any of it beat counting correct answers?**
>
...
## The idea that makes a network worth using here
...
separation at once — trained once, then evaluated for any child in
microseconds, with gradients.

And the reaction-time density is ∂U/∂τ, the same derivative the residual
already contains. The likelihood a child is fitted with comes out of the
differential equation itself.

## The solver is checked before it is used

Two checks, in order. The exact series (Navarro & Fuss, 2009) is verified
against a brute-force simulation of the stochastic process — mean absolute
error 0.014 on densities that peak near 2, and the share of correct responses
...
## The model does not beat counting correct answers
...
## Where the fit is bad, and a wrong explanation
...
distributions, the median child sits at **0.204** — a poor fit, not a marginal
one.
...
child. **The misfit got worse, not better** — median 0.204 → 0.235, improved
for only 30 % of children. The explanation was wrong, and the extra parameters
of a fuller model (across-trial variability in drift and starting point, which
Ratcliff's complete formulation includes and this one omits) are the more
likely cause.
...
- Across-trial variability in drift and starting point — the standard
